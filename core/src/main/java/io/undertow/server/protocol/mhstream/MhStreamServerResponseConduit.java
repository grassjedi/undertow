...
final class MhStreamServerResponseConduit extends AbstractFramedStreamSinkConduit {
    ...
    private final ConduitListener<? super MhStreamServerResponseConduit> finishListener;

    private final boolean headRequest;

    MhStreamServerResponseConduit(final StreamSinkConduit next, final Pool<ByteBuffer> pool, final HttpServerExchange exchange, ConduitListener<? super MhStreamServerResponseConduit> finishListener, boolean headRequest) {
        super(next);
        this.pool = pool;
    ...
    public void setWriteReadyHandler(WriteReadyHandler handler) {
        next.setWriteReadyHandler(new MhStreamServerWriteReadyHandler(handler));
    }
    ...
    boolean doGetRequestBodyChunk(ByteBuffer buffer, final MhStreamServerRequestConduit requestChannel) throws IOException {
    ...
    private final class MhStreamServerWriteReadyHandler implements WriteReadyHandler {

        private final WriteReadyHandler delegate;

        private MhStreamServerWriteReadyHandler(WriteReadyHandler delegate) {
